"""
test_retail_logs.py · El log estructurado y el hilo que cose una petición
=========================================================================
...
⚠️ El nombre del archivo lleva el prefijo `retail` porque las carpetas tests/ no
   son paquetes: pytest importa 'test_logs' a secas y chocaría con el homónimo
   de proyecto_llmops al recolectar. Ver pyproject.toml [tool.pytest.ini_options].
"""
import asyncio
import io
import json
...
class TestFormatoJSON:
    ...
    def test_los_campos_extra_llegan_al_json(self, capturar):
        logs.obtener_logger("prueba").info("request", extra={"ruta": "/chat",
                                                            "estado": 200})
        (linea,) = capturar()
        assert linea["ruta"] == "/chat"
...
class TestRequestId:
    ...
    def test_dos_peticiones_concurrentes_no_se_mezclan(self, capturar):
        ...
        ⚠️ `asyncio.run` desde un test SÍNCRONO: la suite no tiene plugin de
           asyncio (ver pyproject.toml), así que un `@pytest.mark.asyncio` se
           saltaría en silencio — y un test que no corre siempre "pasa".
        """
...
class TestMiddleware:
    ...
    ⭐ POR QUÉ ESTA CLASE EXISTE: los tests de arriba prueban las piezas
       (formateador, ContextVar) y pasarían con el middleware ROTO. El fallo
       clásico es emitir el log DESPUÉS del `finally` que limpia el ContextVar:
       la línea "request" sale sin `request_id` — el único campo por el que se
       investiga. Probar las piezas no prueba el montaje.
    """

    @pytest.fixture
    def cliente(self, raiz_limpia):
    ...
    def _lineas(self, logger="proyecto_retail.app.main") -> list[dict]:
