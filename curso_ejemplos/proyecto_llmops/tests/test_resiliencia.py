...
class TestTimeout:
    ...
    def test_es_configurable_desde_el_entorno(self, monkeypatch):
        ...
        assert llm.crear_llm_cheap().request_timeout == 5.0
...
class TestReintentos:
    ...
    def test_se_pueden_desactivar(self, monkeypatch):
        ...
        assert llm.crear_llm_cheap().root_client.max_retries == 0
...
class TestAmbosModelosDeLaCascada:
    def test_el_modelo_fuerte_tambien_esta_protegido(self):
        # ⚠️ Fácil de olvidar: el fallback es justo el que atiende cuando el
        #    proveedor va MAL. Un strong sin timeout dejaría colgado el camino
        #    de emergencia — el peor sitio posible para no tener límite.
        fuerte = llm.crear_llm_strong()
        assert fuerte.request_timeout == settings.llm_timeout_s
        assert fuerte.root_client.max_retries == settings.llm_max_reintentos
