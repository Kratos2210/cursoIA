"""
logs.py · Logging estructurado: lo que se lee cuando algo falla a las 3 a.m.
============================================================================
FINALIDAD:
  Que un error en producción se pueda INVESTIGAR. Un `print()` en un VPS con
  varios workers produce esto:

      Error al responder
      Error al responder
      procesando pregunta...

  Tres líneas sin dueño. ¿Del mismo usuario? ¿Del mismo worker? ¿Antes o después
  del timeout? No hay forma de saberlo. El log estructurado produce esto otro:

      {"ts":"2026-07-18T20:14:02Z","nivel":"ERROR","mensaje":"Error al responder",
       "request_id":"a3f9c1","rol":"analyst","logger":"app.main"}
...
LÓGICA — las tres decisiones que importan:
  ...
  2) ⭐ EL `request_id` VIAJA EN UN ContextVar, no en un parámetro. Pasarlo a
     ...
     cien requests a la vez. El middleware lo fija al entrar y lo suelta al
...
⚠️ LO QUE NO SE LOGUEA, y es deliberado: la pregunta del usuario y la respuesta
   del modelo. Pueden traer PII (el input_guard anonimiza para el MODELO, pero
   el texto crudo sigue existiendo) y los logs suelen acabar en un tercero que
   no está en el acuerdo de tratamiento de datos. Se loguea la FORMA de la
   petición (rol, longitud, si hubo cache hit), nunca su contenido. Si necesitas
   el contenido para depurar, ahí está Langfuse, que sí es infraestructura tuya.
"""
...
class FormateadorJSON(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Los campos que pasó quien llamó: logger.info("...", extra={"rol": rol}).
...
# Librerías que en INFO narran cada llamada HTTP que hacen. Cargar el modelo de
# embeddings dispara decenas de peticiones al Hub, y todas acabarían en el log
# del servicio: la señal se ahoga en el ruido y el operador deja de leerlo.
# Se las sube a WARNING, así siguen avisando de lo que va MAL.
_RUIDOSOS = ("httpx", "httpcore", "huggingface_hub", "urllib3", "filelock")
